...
from model.sub_module.vit_block import PatchEmbed_ECDP, ViTBlock


class ViT_ECDP(nn.Module):
    def __init__(self, args, input_size=224, patch_size=16, embed_dim=1024,
        ...
        self.patch_embed = PatchEmbed_ECDP(
    ...
    def forward(self, x, mask=False):
        if mask:
            ...
            x = self.patch_embed(x)  # (2,2,224,224)->(2,384,14,14)
            x = x.flatten(2).permute(0, 2, 1)  # (2,196,384)
            # add pos embed w/o cls token
            x = x + self.pos_embed
            x = self.pos_drop(x)
            x = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, x.shape[-1]))  # (2,196,384) -> (2,98,384)
            ...
            x = self.norm_layer(x)
            emb_event, emb_image = x[:, 0], x[:, 1]

            return emb_event, emb_image, mask, ids_restore, attn  # (2,768) (2,196) (2,196) (2,12,100,100)
        else:
            ...
            x = x + self.pos_embed
            x = self.pos_drop(x)
            x = torch.cat((self.tokens.expand(x.shape[0], -1, -1), x), dim=1)
            ...
            for i, blk in enumerate(self.vit_block):
                ...
                if i in self.out_indices:
                    out_embs.append(emb2patch_frame(x[:, 2:, :]))
...
def vit_ecdp_small_patch16(args, **kwargs):
    model = ViT_ECDP(args=args,
...
def vit_ecdp_base_patch16(args, **kwargs):
    model = ViT_ECDP(args=args,
