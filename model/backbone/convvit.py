...
class ConvViT(nn.Module):
    ...
    def forward(self, x, mask=False):
        if mask:
            ...
            emb_l1 = x
            emb_stage1 = self.stage1_output_decode(x).flatten(2).permute(0, 2, 1)  # (2,196,384)
            emb_stage1 = torch.gather(emb_stage1, dim=1,
                                      index=ids_keep.unsqueeze(-1).repeat(1, 1, emb_stage1.shape[-1]))
            ...
            emb_l2 = x
            emb_stage2 = self.stage2_output_decode(x).flatten(2).permute(0, 2, 1)  # (2,196,384)
            emb_stage2 = torch.gather(emb_stage2, dim=1,
                                      index=ids_keep.unsqueeze(-1).repeat(1, 1, emb_stage2.shape[-1]))
            ...
            if self.args.use_feature_fusion:
                emb_lh = self.norm_layer(emb_stage1 + emb_stage2 + emb_stage3)
            else:
                emb_lh = self.norm_layer(emb_stage3)

            return emb_l1, emb_l2, emb_lh, mask, ids_restore
            ...
            if self.args.phase == "finetune_semseg" or self.args.phase == "finetune_flow":
                emb_h1 = emb2patch_frame(emb_h)
                # down sample
                emb_h2 = self.fpn(emb_h1)  # (b,384,14,14) -> (b,384,6,6)
                out_embs = [emb_l1, emb_l2, emb_h1, emb_h2]  # (b,128,56,56) (b,256,28,28) (b,384,14,14) (b,384,6,6)
                return emb_l1, emb_l2, emb_h, out_embs, attn
            else:
                return emb_l1, emb_l2, emb_h, attn


def convvit_small_patch16(args, **kwargs):
    model = ConvViT(args=args,
...
def convvit_base_patch16(args, **kwargs):
    model = ConvViT(args=args,
        input_size=[224, 56, 28], patch_size=[4, 2, 2],
