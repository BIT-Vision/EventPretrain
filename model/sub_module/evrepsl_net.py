...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding="same", padding_mode=get_padding_mode(),
                      bias=False),
            default_bn(mid_channels),
            default_act(),

            nn.Conv2d(mid_channels, out_channels, kernel_size=3, padding="same", padding_mode=get_padding_mode(),
                      bias=False),
...
class OutIE(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.double_conv = nn.Sequential(
            ...
            nn.Conv2d(mid_channels, mid_channels, kernel_size=3, padding="same", padding_mode=get_padding_mode(),
                      bias=False),
            default_bn(mid_channels),
            default_act(),

            nn.Conv2d(mid_channels, out_channels, kernel_size=1, padding="same", padding_mode=get_padding_mode(),
                      bias=False),
            # default_bn(out_channels),
            # default_act()
        )
...
class EffNet(nn.Module):
    def __init__(self, n_channels, out_depth, inc_f0=0, bilinear=False, n_lyr=4, ch1=24, out_cnn=True, c_is_const=False,
                 c_is_scalar=False, device="cuda:1"):
        ...
        self.mparams = {"n_lyr": n_lyr, "bilinear": bilinear, "n_chs": n_chs, "n_rep_dn": n_rep_dn, "lyr_ts": lyr_ts,
                        "n_rep_up": n_rep_up, "expans": expans, "pool_szs": pool_szs, "factor": factor}
        ...
        for i in range(n_lyr):
            ...
            lyr = DownMB(n_chs[i] if i != 0 else n_chs[0] * double_factor, out_chnl, lyr_ts[i], expansion=expans[i],
                         n_repeats=n_rep_dn[i], pool_size=pool_szs[i])
...
class EffWNet(nn.Module):
    def __init__(self, n_channels, out_depth, inc_f0=1, bilinear=False, n_lyr=4, ch1=24, c_is_const=False,
                 c_is_scalar=False, device="cuda:1"):
        ...
        for i in range(n_lyr):
            out_chnl = n_chs[i + 1] // factor if i == n_lyr - 1 else n_chs[i + 1]
            lyr = DownMB(n_chs[i], out_chnl, lyr_ts[i], expansion=expans[i], n_repeats=n_rep_dn[i],
                         pool_size=pool_szs[i])
            self.downs.append(lyr)
    ...
    def ups_builder(self):
        ...
        for i in range(self.mparams["n_lyr"]):
            ...
            out_chnl = self.mparams["n_chs"][rev_i] if i == self.mparams["n_lyr"] - 1 else self.mparams["n_chs"][
                                                                                               rev_i] // self.mparams[
                                                                                               "factor"]

            lyr = UpMB(self.mparams["n_chs"][rev_i + 1], out_chnl, self.mparams["lyr_ts"][rev_i],
                       expansion=self.mparams["expans"][rev_i], n_repeats=self.mparams["n_rep_up"][i],
                       bilinear=self.mparams["bilinear"], scale_factor=self.mparams["pool_szs"][rev_i])
            ups.append(lyr)
...
def evrepsl_net():
    model = EffWNet(n_channels=3, out_depth=1, inc_f0=1, bilinear=True, n_lyr=4, ch1=12, c_is_const=False, c_is_scalar=False)
    return model
